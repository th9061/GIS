 ...
 */
#define FONT_CACHE_SIZE_FOR_ZEROS 40000
#define FONT_CACHE_SIZE_FOR_ROTATED 40000
...
int getZoomLevel(){ return zoomlvl.level;}
...
handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    if(zoomlvl.level + 1 == 11) return;
    zoomlvl.level++;
    //cout << zoomlvl.level << endl;
...
panning_execute(int x, int y, void (*drawscreen) (void)) {
    ...
    /*float xl = trans_coord.xleft - x_change_world;
    float xr = trans_coord.xright - x_change_world;
    float yb = trans_coord.ybot - y_change_world;
    float yt = trans_coord.ytop - y_change_world;
    
    if(xl < map_bounds.minlon*LAT_TO_COOR_RATIO || 
            xr > map_bounds.maxlon*LAT_TO_COOR_RATIO) 
        return;
    if(yb < map_bounds.minlat*LAT_TO_COOR_RATIO || 
            yt > map_bounds.maxlat*LAT_TO_COOR_RATIO) 
        return;*/
                ...
                if (report.type == ButtonPress) {
                    ...
                    switch (report.xbutton.button) {
                        case Button4:
                            ...
                            else if(path_set && dir_bounds.intersects(mouse_location_current) && !dir.hide)
                            {
                                if(dir.scroll_count > 0 && (int)dir.dir_parsed.size() > dir.num_div)
                                {
                            ...
                            else if(legend_state && layermenu_bounds.intersects(mouse_location_current))
                            {
                                if(legend.scroll_count > 0 && (int)legend.active_layers.size() - 1 > legend.num_div - 1)
                                {
                                    legend.scroll_count = legend.scroll_count - 1;
                                    draw_layermenu(get_scale());
                                }                                   
                            }
                            else if(dir.hide && dir.header.intersects(mouse_location_current));
                            else if(getZoomLevel() > 0 && minimap_bounds.intersects(mouse_location_current));
                            else handle_zoom_in(x, y, NULL); // (same function as normal event logic)
                            break;
                        case Button5:
                            if(menu.active_button == 0);
                            else if(path_set && dir_bounds.intersects(mouse_location_current) && !dir.hide)
                            {
                                if(dir.scroll_count + dir.num_div +3 < (int)dir.dir.size()
                                        && path_set && (int)dir.dir_parsed.size() > dir.num_div)
                                {
                                    dir.scroll_count = dir.scroll_count + 1;
                                    if(dir.spec)
                                    {
                            ...
                            else if(legend_state && layermenu_bounds.intersects(mouse_location_current))
                            {
                                if(legend.scroll_count + legend.num_div < (int)legend.active_layers.size()
                                        && legend_state && (int)legend.active_layers.size() - 1 > legend.num_div - 1)
                                {
                                    legend.scroll_count = legend.scroll_count + 1;
                                    draw_layermenu(get_scale());
                                }
                            ...
                            else handle_zoom_out(x, y, NULL); // (same function as normal event logic)
                            break;
                        default:
...
#endif
                    switch (report.xbutton.button) {
                        case Button1: left_hold = true;
                                        //cout << "left click" << endl; /* Left mouse click; pass back to client program */
                        ...
                        case Button2: /* Scroll wheel pressed; start panning */
                            if(path_set && dir_bounds.intersects(mouse_location_current) && !dir.hide);
                            else if(legend_state && layermenu_bounds.intersects(mouse_location_current));
                            else if(dir.hide && dir.header.intersects(mouse_location_current));
                            else if(getZoomLevel() > 0 && minimap_bounds.intersects(mouse_location_current));
                            else panning_on(report.xbutton.x, report.xbutton.y);
                            break;
                        case Button4: /* Scroll wheel rotated forward; screen does zoom_in */
                            ...
                            else handle_zoom_in(x, y, drawscreen);
                            break;
                        case Button5: /* Scroll wheel rotated backward; screen does zoom_out */
                            ...
                            else handle_zoom_out(x, y, drawscreen);
                            break;
                    }
            ...
            case KeyPress:
                ...
                if (gl_state.get_keypress_input) {
                    ...
                    length = XLookupString(&report.xkey, keyb_buffer, max_bytes, &keysym,
                        &composestatus);
                    if (keysym == XK_Left)
                        act_on_key_press(200);
                    else if (keysym == XK_Right)
                        act_on_key_press(201);
                    else if (keysym == XK_Up)
                        act_on_key_press(202);
                    else if (keysym == XK_Down)
                        act_on_key_press(203);
